...
def main(args):
    ...
    for file in face_files:
        ...
        # Create a backup of the original data files
        shutil.copyfile(f'{args.data}/{file}/images.npy', f'{args.data}/{file}/images_backup.npy')
        shutil.copyfile(f'{args.data}/{file}/gazes.npy', f'{args.data}/{file}/gazes_backup.npy')

        # Remove the selected indices from the original data arrays
        image_arrays = np.delete(image_arrays, selected_indices, axis=0)
        label_arrays = np.delete(label_arrays, selected_indices, axis=0)

        # Save the modified original data arrays back to the original files
        np.save(f'{args.data}/{file}/images.npy', image_arrays)
        np.save(f'{args.data}/{file}/gazes.npy', label_arrays)
